import {
  MACHINE_OPTIONS, FUEL_THEFT_LEVELS, ESCALATION, CALC_DEFAULTS,
...
export default function PrintInputsPage({ inputs, pageNumber, pageCount }) {
  ...
  const option = MACHINE_OPTIONS.find((o) => o.id === inputs.machineOption) ?? MACHINE_OPTIONS[1];
  ...
  return (
    <PrintPage pageNumber={pageNumber} pageCount={pageCount}>
      ...
      <SpecRows rows={[
        { label: 'Machine option compared', value: option.label },
        { label: 'Fleet size', value: `${inputs.fleetSize} machine${inputs.fleetSize > 1 ? 's' : ''} — all costs scale ×${inputs.fleetSize}` },
