...
export default function PrintComparisonPage({ selection, inputs, pageNumber, pageCount }) {
  ...
  // Every opponent running at the hero's exact cost per hour (the two diesel
  // brake variants) — the comparison is purely the purchase-price gap.
  const allSameRunning = hasComparison && comparisons.every((c) => c.sameRunningCosts);
  ...
  const comparisonIntro = allSameRunning
    ? 'The selected machines are compared over the same operating hours with your utilization, duty cycle and energy prices. These variants run at identical cost per hour — the comparison comes down to purchase price.'
  ...
  return (
    <PrintPage pageNumber={pageNumber} pageCount={pageCount}>
      ...
      {hasComparison && (
        <>
          <Band>{allSameRunning ? 'The difference in price' : 'Where the lines cross'}</Band>
          <SpecRows rows={comparisons.flatMap((c) => c.sameRunningCosts
            ? [
                {
                  label: `Price difference — ${variantName(c.machine)} vs ${variantName(heroMachine)}`,
                  value: `${formatCurrency(c.priceGapFleet)} upfront`,
                },
                {
                  label: `Running cost / h vs ${variantName(heroMachine)}`,
                  value: 'Identical — the price gap holds over the full lifecycle',
                },
              ]
            : [
                {
                  label: `Savings start — ${variantName(heroMachine)} vs ${variantName(c.machine)}`,
                ...
                {
                  label: `Simple year-0 check vs ${variantName(c.machine)} (price gap ÷ hourly saving)`,
            ...
            <div className="print-callout">
              ...
              <p className="print-callout__text">
                {best.sameRunningCosts
                  ? `The ${heroMachine.displayName} costs ${formatCurrency(best.priceGapFleet)} less to buy than the ${best.machine.displayName} for ${inputs.fleetSize} machine${inputs.fleetSize > 1 ? 's' : ''} and runs at the same cost per hour — the saving holds across the full ${formatHours(maxHours)}.`
                  : `Total cost of ownership advantage of the ${heroMachine.displayName} over the ${best.machine.displayName} for ${inputs.fleetSize} machine${inputs.fleetSize > 1 ? 's' : ''}, including purchase price, energy and service.`}
              </p>
            </div>
          )}
        </>
