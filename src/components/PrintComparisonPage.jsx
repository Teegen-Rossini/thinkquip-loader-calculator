...
import { applyVat, savingsAtHours } from '../lib/calculationEngine';
...
function MachineColumn({ machine, result, inputs, isElectric, breakevenNote }) {
...
/** Brochure section 3 — the two SANY machines side by side, ending in the
 *  headline savings callout. */
export default function PrintComparisonPage({ comparison, inputs, pageNumber, pageCount }) {
  const { electric, diesel, electricMachine, dieselMachine, breakevenHours, simpleBreakevenHours, hoursPerYear } = comparison;
  ...
  return (
    <PrintPage pageNumber={pageNumber} pageCount={pageCount}>
      ...
      <div className="print-compare">
        <MachineColumn machine={electricMachine} result={electric} inputs={inputs} isElectric />
        <MachineColumn machine={dieselMachine} result={diesel} inputs={inputs} isElectric={false} />
      </div>

      <Band>Where the lines cross</Band>
      <SpecRows rows={[
        {
          label: 'Savings start (with escalation)',
          value: hasBreakeven
            ? `${formatHours(breakevenHours)} — ~${formatYearsFromHours(breakevenHours, hoursPerYear)} at your hours`
            : `Beyond ${formatHours(maxHours)} at these inputs`,
        },
        {
          label: 'Simple year-0 check (price gap ÷ hourly saving)',
          value: simpleBreakevenHours != null ? formatHours(simpleBreakevenHours) : 'No year-0 hourly saving at these inputs',
        },
      ]} />

      {savings != null && savings > 0 && (
        <div className="print-callout">
          <p className="print-callout__value">{formatCurrency(savings)} saved by {formatHours(maxHours)}</p>
          <p className="print-callout__text">
            Total cost of ownership advantage of the {electricMachine.displayName} over the {dieselMachine.displayName} for{' '}
