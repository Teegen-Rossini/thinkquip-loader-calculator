...
/**
 * Brochure section 5 — one full A4 spec sheet per SANY machine, modelled on
 * ThinkQuip's physical printed sheets: brand header, machine cutout on a
 * tinted hero panel with the price, then banded two-column spec sections.
 * All figures come from machinesConfig.js / the calculation engine.
 */
export default function PrintSpecSheet({ machine, comparison, pageNumber, pageCount, isLast }) {
  const isElectric = machine.type === 'electric';
  const per = comparison[isElectric ? 'electric' : 'diesel'].perHour;
  const battery = comparison.battery;
  ...
  // First block sits alone in column flow; keep hero + header out of columns.
  return (
