 ...
 *  static print SVG, its sampled data table, and the battery-replacement
 *  callout (which lands beyond the chart and is deliberately not plotted). */
export default function PrintTimelinePage({ comparison, inputs, pageNumber, pageCount }) {
  const { electric, diesel, electricMachine, dieselMachine, battery, hoursPerYear } = comparison;
    ...
    <PrintPage pageNumber={pageNumber} pageCount={pageCount}>
      ...
      <div className="print-chart">
        <div className="print-chart__legend">
          <span className="print-chart__legend-item">
            <span className="print-chart__legend-swatch" style={{ background: electricMachine.chartColor }} />
            {electricMachine.displayName}
          </span>
          <span className="print-chart__legend-item">
            <span className="print-chart__legend-swatch" style={{ background: dieselMachine.chartColor }} />
            {dieselMachine.displayName}
          </span>
        </div>
        <PrintChart comparison={comparison} />
      </div>
      ...
      <table className="print-table">
        <thead>
          <tr>
            ...
            <th>≈ Calendar year</th>
            <th className="num">{electricMachine.name} cumulative</th>
            <th className="num">{dieselMachine.name} cumulative</th>
            <th className="num">Electric advantage</th>
          </tr>
        </thead>
        <tbody>
          {sampleHours.map((h) => {
            const saving = savingsAtHours(electric.series, diesel.series, h);
            return (
              <tr key={h}>
                <td>{formatHours(h)}</td>
                <td>{formatYearsFromHours(h, hoursPerYear)}</td>
                <td className="num">{formatCurrency(cumulativeCostAtHours(electric.series, h))}</td>
                <td className="num">{formatCurrency(cumulativeCostAtHours(diesel.series, h))}</td>
                <td className="num">{saving > 0 ? formatCurrency(saving) : `−${formatCurrency(Math.abs(saving))}`}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <div className="print-callout">
        ...
        <p className="print-callout__text">
          ...
          and the first owner's typical lifecycle, so it is not on the curve above. Projected cost per machine when it
